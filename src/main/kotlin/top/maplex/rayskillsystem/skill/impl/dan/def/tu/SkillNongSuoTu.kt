...
object SkillNongSuoTu : AbstractSkill, YuanSu, AbstractDanCast {
    @Awake(LifeCycle.LOAD)
    fun onEnable() {
        register()
    }

    override val name: String = "浓缩土元素"
    ...
    override fun onRun(player: Player, level: Int): Boolean {
        TargetRange.get(player, 11.0, true).forEach {
            if (Team.canAttack(player, it)) {
                //敌人
                BuffManager.add(it, BuffNoMove.id, 1, 20 * 10, player.uniqueId)
            } else {
                it.removePotionEffect(PotionEffectType.ABSORPTION)
                it.addPotionEffect(PotionEffectType.ABSORPTION.createEffect(1800 * 20, 8))
            }
        }
        return true
    }
